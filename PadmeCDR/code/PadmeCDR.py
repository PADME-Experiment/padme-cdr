...
sites_list = [ "ALL", "DAQ", "LNF", "CNAF", "CNAF2", "KLOE" ]
source_sites_list = [ "DAQ", "LNF", "CNAF", "CNAF2" ]
destination_sites_list = [ "LNF", "CNAF", "CNAF2", "KLOE" ]

# List of data types that can be tranfered. Affects path in the PADME tree structure
data_types_list = [ "DAQ", "MM", "TMM" ]

# Define list of years of data taking
years_list = [ "2018", "2019", "2020", "2021", "2022", "2023", "2024", "2025" ]

def print_help():
    print 'PadmeCDR [-T data_type] [-S src_site -D dst_site] [-L site] [-s data_srv] [-Y year] [-a after] [-b before] [-i] [-h]'
    print '  -T data_type    Data type to transfer %s'%data_types_list
...
def main(argv):
    ...
    try:
        opts,args = getopt.getopt(argv,"T:S:D:s:Y:a:b:ih")
    ...
    # Check if data type was defined and is allowed
    if not data_type:
        print "ERROR - Data type was not defined. Use one of",data_types_list
        print_help()
        sys.exit(2)
    elif not data_type in data_types_list:
        print "ERROR - Data type",data_type,"is not available. Use one of",data_types_list
        print_help()
        sys.exit(2)
    ...
    if ( source_site == "DAQ" and data_type == "DAQ" and not (data_server in data_servers_list) ):
        if (data_server):
    ...
    if (source_site == "DAQ"):
        print "Starting PadmeCDRServer for",data_type,"with source",source_site,"server",data_server,"and destination",destination_site
    else:
        print "Starting PadmeCDRServer for",data_type,"with source",source_site,"and destination",destination_site
    ...
    proxy_cmd = "voms-proxy-init --valid 720:00 --out %s"%long_proxy_file
    print ">",proxy_cmd
    ...
    if serverInteractive:
        PadmeCDRServer(data_type,source_site,destination_site,data_server,year,date_after,date_before,"i")
    else:
        print "Starting PadmeCDRServer in background"
        with daemon.DaemonContext(working_directory="."): PadmeCDRServer(data_type,source_site,destination_site,data_server,year,date_after,date_before,"d")
