...
class PadmeCDRServer:
    ...
    def __init__(self,data_type,source_site,destination_site,daq_server,year,date_after,date_before,mode):
        ...
        # If source is an online node and the daq server was not specified, set it here
        if self.src_site == "DAQ":
            if self.data_type == "DAQ" and not self.daq_server:
                    print "ERROR - Data type is DAQ and source site is DAQ but no source server was specified. Aborting"
                    sys.exit(2)
            elif self.data_type == "MM" and not self.daq_server:
                self.daq_server = "l0padme2"
                print "INFO - Data type is MM and source site is DAQ but no source server was specified: will use %s"%self.daq_server
            elif self.data_type == "TMM" and not self.daq_server:
                self.daq_server = "192.168.60.10"
                print "INFO - Data type is TMM and source site is DAQ but no source server was specified: will use %s"%self.daq_server

        # Define id of this server to be used for service files
        self.server_id = ""
        if self.src_site == "DAQ":
            self.server_id += "%s_%s_%s_%s"%(self.data_type,self.src_site,self.daq_server,self.dst_site)
        else:
            self.server_id += "%s_%s_%s"%(self.data_type,self.src_site,self.dst_site)
        ...
        # Create lock file
        self.lock_file = "%s/run/PadmeCDRServer_%s.lock"%(self.cdr_dir,self.server_id)
        if (self.create_lock_file() == "error"): exit(1)

        # Create proxy handler
        self.PH = ProxyHandler()
        self.PH.long_proxy_file = "%s/run/long_proxy"%self.cdr_dir
        self.PH.debug = 1

        # Path to stop_cdr file: when file appears, server will remove it and gently exit
        self.stop_cdr_file = "%s/run/PadmeCDRServer_%s.stop"%(self.cdr_dir,self.server_id)

        # User running CDR
        self.cdr_user = os.environ['USER']
        self.cdr_home = os.environ['HOME']

        # Define minimum duration for an iteration (4 hours = 14400 seconds)
        self.iteration_minimum_duration = 14400

        # Path to current_run and last_run files on online data servers
        self.current_run_file = "/home/daq/DAQ/run/current_run"
        self.last_run_file = "/home/daq/DAQ/run/last_run"

        # Define data path according to year and data type
        if self.data_type == "DAQ":
            self.data_dir = "%s/rawdata"%self.year
        elif self.data_type == "MM":
            self.data_dir = "%s/mmdata"%self.year
        elif self.data_type == "TMM":
            self.data_dir = "%s/tmmdata"%self.year
        ...
        self.daq_user = "daq"
        self.daq_keyfile = "%s/.ssh/id_rsa_cdr"%self.cdr_home
        ...
        self.kloe_user = "pdm"
        self.kloe_keyfile = "%s/.ssh/id_rsa_cdr"%self.cdr_home
        ...
        self.site_srm = {
            "LNF": "root://atlasse.lnf.infn.it//dpm/lnf.infn.it/home/vo.padme.org/daq",
            "CNAF": "davs://xfer-archive.cr.cnaf.infn.it:8443/padmeTape/daq",
            "CNAF2": "davs://xfer-archive.cr.cnaf.infn.it:8443/padme/daq"
        }
    ...
    def get_ongoing_run(self):
        ...
        print "Getting on-going run from online server %s"%self.daq_server
        ...
        if rc == 0:
            ...
            print "- WARNING - Command returned error %d\n%s"%(rc,err)
    ...
    def get_run_list_daq(self):
        ...
        print "Getting list of runs for year %s on %s server %s"%(self.year,self.data_type,self.daq_server)
        cmd = "%s \'( cd %s/%s; ls -d run_* )\'"%(self.daq_ssh,self.daq_path,self.data_dir)
        (rc,out,err) = self.execute_command(cmd)
        if rc == 0:
            for line in iter(out.splitlines()):
                run = line.rstrip()
                if (re.match("run_\d+_\d+_\d+",run) and run != self.ongoing_run):
                    run_list.append(run)
        else:
            print "***ERROR*** remote command returned error status %d while retrieving run list\n%s"%(rc,err)
            return [ "error" ]
    ...
    def get_run_list_srm(self,site):
        ...
        if rc == 0:
            for line in iter(out.splitlines()):
                run = line.rstrip()
                if re.match("run_\d+_\d+_\d+",run): run_list.append(run)
        else:
            print "***ERROR*** gfal-ls returned error status %d while retrieving run list from %s\n%s"%(rc,site,err)
            return [ "error" ]
    ...
    def get_file_list(self,site,run):
        if (site == "DAQ"):
            ...
            return self.get_file_list_srm("CNAF2",run)

    def get_file_list_daq(self,run):

        print "Getting list of raw data files for run %s on %s server %s"%(run,self.data_type,self.daq_server)
        ...
        if rc == 0:
            ...
            print "***ERROR*** remote command returned error status %d while retrieving file list\n%s"%(rc,err)
            return [ "error" ]
    ...
    def get_file_list_kloe(self,run):
        ...
        # If no files are found (error 8), just leave the list empty
            ...
            print "Error %d while accessing KLOE tape library\n%s"%(rc,err)
    ...
    def get_checksum_srm(self,site,rawfile):
        ...
        if rc == 0:
            ...
            print "- WARNING - gfal-sum returned error %d\n%s"%(rc,err)
        return a32
    ...
    def get_checksum_daq(self,rawfile):
        ...
        if rc == 0:
            ...
            print "- WARNING - DAQ adler32 command returned error %d\n%s"%(rc,err)
        return a32
    ...
    def copy_file_daq_srm(self,dst_site,rawfile):

        self.PH.renew_voms_proxy()

        copy_failed = False

        print "- File %s - Starting copy from DAQ to %s"%(rawfile,dst_site)
        ...
        if rc == 0:
            ...
            print "- File %s - ***ERROR*** gfal-copy returned error %d while copying from DAQ to %s"%(rawfile,rc,site)
            print err,
        ...
        a32_dst = self.get_checksum_srm(dst_site,rawfile)
        print "- File %s - ADLER32 CRC - Source: %s - Destination: %s"%(rawfile,a32_src,a32_dst)
        if ( a32_src == "" or a32_dst == "" or a32_src != a32_dst ):
            print "- File %s - ***ERROR*** unmatched checksum while copying from DAQ to %s"%(rawfile,dst_site)
            with open(self.transfer_error_list_file,"a") as telf:
    ...
    def copy_file_srm_srm(self,src_site,dst_site,rawfile):
        ...
        if rc == 0:
            ...
            print "- File %s - ***ERROR*** gfal-copy returned error %d while copying from %s to %s"%(rawfile,rc,src_site,dst_site)
            print err,
            with open(self.transfer_error_list_file,"a") as telf:
                telf.write("%s - %s copy\n"%(self.now_str(),rawfile))
            cmd = "gfal-rm %s/%s/%s"%(self.site_srm[dst_site],self.data_dir,rawfile)
            ...
            if rc == 0:
                ...
                print "- File %s - ***ERROR*** gfal-rm returned error %d while deleting destination copy at %s"%(rawfile,rc,dst_site)
                print err,
            return "error"
    ...
    def copy_file_srm_kloe(self,site,rawfile):
        ...
        if rc == 0:
            ...
            print "- File %s - ***ERROR*** gfal-copy returned error %d while copying from %s to local file\n%s"%(rawfile,rc,site,err)
            with open(self.transfer_error_list_file,"a") as telf:
                telf.write("%s - %s copy\n"%(self.now_str(),rawfile))
    ...
    def copy_file(self,src_site,dst_site,rawfile):
        ...
        # DAQ -> LNF/CNAF/CNAF2
        if (src_site == "DAQ"):
            if (dst_site == "LNF" or dst_site == "CNAF" or dst_site == "CNAF2"):
                return self.copy_file_daq_srm(dst_site,rawfile)
        ...
        # LNF -> CNAF/CNAF2/KLOE
            ...
            if (dst_site == "CNAF"):
                ...
                return self.copy_file_srm_srm("LNF","CNAF2",rawfile)
            elif (dst_site == "KLOE"):
        ...
        # CNAF -> LNF/CNAF2/KLOE
            ...
            if (dst_site == "LNF"):
                ...
                return self.copy_file_srm_srm("CNAF","CNAF2",rawfile)
            elif (dst_site == "KLOE"):
        ...
        # CNAF2 -> LNF/CNAF/KLOE
            ...
            if (dst_site == "LNF"):
                return self.copy_file_srm_srm("CNAF2","LNF",rawfile)
            elif (dst_site == "CNAF"):
                return self.copy_file_srm_srm("CNAF2","CNAF",rawfile)
            elif (dst_site == "KLOE"):
                return self.copy_file_srm_kloe("CNAF2",rawfile)
